...
class graph_defaults:
    ...
    def __init__(self, x, tdata, data, aprox_data, distance):
        self.x = x
        self.tdata = tdata
        self.data = data
        self.aprox_data = aprox_data
        self.distance = distance


    def graph_distance(self, name='stability'):
        """
        ...
        plt.plot(self.tdata, self.distance)
        plt.xlim(self.tdata[0], self.tdata[len(self.tdata) - 1])
    ...
    def graph_time(self, times, name='Approximation_t='):
        times = np.array(self.tdata)
        for i in times:
            fig, ax = plt.subplots()
            plt.rc('text', usetex=True)
            plt.rc('font', family='serif')
            ...
            plt.title('Approximation with chebycheb polynomials',
                      fontsize=16, color='black')
            ax.grid()
            ax.plot(self.x, self.data[self.tdata.index(i), :], 'r', label='Approximation')
            ax.plot(self.x, self.aprox_data[self.tdata.index(i), :], 'b', label=r'\textit{U} (x)')
            ax.legend()
    ...
    def graph_3d(self, name='solution'):
        ...
        """
        X, Y = np.meshgrid(self.x, self.tdata)
