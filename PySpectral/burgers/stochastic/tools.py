...
class JNM:
    ...
    def J_NM(self, M, N1):
        ...
        for k in range(1, N1):
            h = list(itertools.combinations(range(3, self.N + 1), k))
            L = len(h)
            L2 = L1 + L - 1
            J[0, L1 - 1:L2] = 1
            J[1, L1 - 1:L2] = 2
...
class analysis:
    ...
    def __init__(self, **params):
        self.nu = params['nu'], self.N = params['N']
        self.xSpace = params['xSpace'], self.tim = params['tim']
    ...
    def integ(self):
        ...
        Pn = np.zeros([self.M, self.M])
        Qn = np.zeros([self.M, self.M])
        for j in range(0, self.M):
            for i in range(0, self.M):
                if self.J[i, j] > 0:
                    Jij = int(self.J[i, j])
                    P = lambda x, y: (abs(
                        ((-1) ** Jij) * (special.eval_hermitenorm(Jij, x) - special.eval_hermitenorm(Jij, y)) / (
                                    factorial(Jij + 1) ** 2)) ** 2) * measure(x) * measure(y)
                    Q = lambda x, y: (abs(
                        ((-1) ** Jij) * special.eval_hermitenorm(Jij, y) / (factorial(Jij + 1) ** 2)) ** 2) * measure(
                        x) * measure(y)
        ...
        for k in range(0, len(self.tim)):
            ...
            for j in range(0, self.M):
                for i in range(0, self.M):
                    prod1 = 1.0
                    prod2 = 1.0
                    sum1 = Ut[self.J[i, j]] ** 2
                    sum2 = (Ut[self.J[i, j]] - Vt[self.J[i, j]]) ** 2
