...
class integrator:
    ...
    def __init__(self, u0, params):
        ...
        self.N = params['N']
        self.Q = params['Q']
        self.xSpace = params['xSpace']
        self.tim = params['tim']
        self.J = params['J'], self.M = params['M']
        self.rulesX = params['rulesX'], self.rulesW = params['rulesW']
        self.LRules = params['LRules']
        ...
        def make(f):
            ...
            def constants(xSpace, M, J, rulesX, rulesW, nu):
                ...
                for z in range(0, Lx):
                    ...
                    return ci
            return constants
        return make(jit(self.u0))(self.xSpace, self.M, self.J, self.rulesX, self.rulesW, self.nu)
    ...
    def SimulaX(self):
        ...
        return make(evalXSin)(self.xSpace, self.M, self.J)


class set_simulation(integrator):

    def __init__(self, u0, params):
        super(set_simulation, self).__init__(u0, **params)


    @jit
    def SimulaT(self, cons):
        """
        ...
        Pt = len(self.tim)
        T = np.zeros([Pt, self.M])
        for k in range(0, Pt):  ### for each point t
            prod = np.zeros(self.M)

            for j in range(0, self.M):  ### for each multi-index n_j\in J^{N,M}
                sum = 0
                for i in range(0, self.M):  ### for each member inside the multi-index n_j=(\alpha_i,,,,)
                    if self.EigValIm[i] != 0:
                        if i == 1:
                            sum = sum + cons[i] * exp(self.EigValRe[i] * self.tim[k] / pi ** 2) * (
                                    self.EigVecRe[j, i] * cos(self.EigValIm[i] * self.tim[k] / pi ** 2) - self.EigVecIm[
                                j, i] * sin(
                                self.EigValIm[i] * self.tim[k] / pi ** 2))
                    if i > 1:
                        if self.EigValIm[i] != - self.EigValIm[i - 1]:
                            sum = sum + cons[i] * exp(self.EigValRe[i] * self.tim[k] / pi ** 2) * (
                                    self.EigVecRe[j, i] * cos(self.EigValIm[i] * self.tim[k] / pi ** 2)
                                    - self.EigVecIm[j, i] * sin(self.EigValIm[i] * self.tim[k] / pi ** 2))
                    if self.EigValIm[i] == - self.EigValIm[i - 1]:
                        sum = sum + cons[i] * exp(self.EigValRe[i] * self.tim[k] / pi ** 2) * (
                                self.EigVecRe[j, i - 1] * sin(self.EigValIm[i - 1] * self.tim[k] / pi ** 2) + self.EigVecIm[
                            j, i - 1] * cos(self.EigValIm[i - 1] * self.tim[k] / pi ** 2))

                    if self.EigValIm[i] == 0:
                        sum = sum + cons[i] * self.EigVecRe[j, i] * exp(self.EigValRe[i] * self.tim[k] / pi ** 2)
                prod[j] = sum
            T[k, :] = prod
    ...
    def SimulaTX(self, x):
        ...
        """
        TX = np.zeros([len(self.tim), len(x)])
        for z in range(0, len(x)):
            TX[:, z] = np.dot(self.SimulaT(np.dot(np.linalg.inv(self.EigVecRe), self.U_1[:, z])), self.H1)[:, z]
