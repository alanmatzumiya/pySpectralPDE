...
def run_burgers():
    """
    Solves the Burgers Stochastics equation on 1D using Spectral method
    ...
    # Discretization
    xSpace = np.linspace(0,1, 1024)
    tim = np.linspace(0, 3, 512)
    ...
    # Graphics creating
    Graph(xSpace, tim, simulation1, simulation2, u0, aprox)

    # compute convergence
    conv_IC(xSpace, tim, M, J, EigValRe, EigValIm, EigVecRe, EigVecIm, U_1, U_2)
